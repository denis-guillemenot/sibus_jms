# ===========================================
# ./wsadmin.sh -lang jython -host <dmgr_host> -port <dmgr_port> -f sibus_jms_queue_writer_soap.py sampleQueue bla bla bla
#   version 1.1
# ===========================================
script_name = "sibus_queue_jms_writer_soap.py"
...
def print_usage():
  print('\nUsage: wsadmin.sh -conntype SOAP -lang jython -f <%s> <ConnectionFactory> <QueueName> [message]' % script_name)
...
def queue_writer( *argv):
  """
    Write a TEXT message to a JMS Queue
  """
  ...
  if len( argv) >= 2:
    ...
    queue_name = argv[1]
    if len( argv) > 2: 
      queue_message = ' '.join( argv[2:])
    else:
      queue_message = "QUEUE: Just a test with Jython"
    print('Writing message to [jms/%s]' % queue_name)
    print('Message is: %s ' % queue_message)
  else:
    print_usage()
    return
  ...
  qsender = session.createProducer( qdestination)
  qsender.send( session.createTextMessage( queue_message))
